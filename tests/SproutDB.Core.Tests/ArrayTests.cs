...
public class ArrayTests : IDisposable
{
    ...
    public ArrayTests()
    {
        ...
        _engine.ExecuteOne("create table users (name string 100, roles array string 30)", "testdb");
    }
    ...
    {
        _engine.ExecuteOne("add column users.tags array string 50", "testdb");
        var result = _engine.ExecuteOne("describe users", "testdb");
    ...
    {
        var result = _engine.ExecuteOne("upsert users {name: 'Alice', roles: ['admin', 'editor']}", "testdb");
    ...
    {
        _engine.ExecuteOne("upsert users {name: 'Bob', roles: ['viewer']}", "testdb");
    ...
    {
        var result = _engine.ExecuteOne("upsert users {name: 'Charlie', roles: []}", "testdb");
        Assert.Equal(SproutOperation.Upsert, result.Operation);
    ...
    {
        _engine.ExecuteOne("upsert users {name: 'Dave', roles: ['x']}", "testdb");

        // Update to null
        var result = _engine.ExecuteOne("get users where name = 'Dave'", "testdb");
        var id = result.Data?[0]["_id"];
        Assert.NotNull(id);

        var update = _engine.ExecuteOne($"upsert users {{_id: {id}, roles: null}}", "testdb");
        Assert.Equal(SproutOperation.Upsert, update.Operation);

        var getResult = _engine.ExecuteOne($"get users where _id = {id}", "testdb");
    ...
    {
        _engine.ExecuteOne("upsert users {name: 'Eve', roles: ['admin', 'user']}", "testdb");
        _engine.ExecuteOne("upsert users {name: 'Frank', roles: ['user']}", "testdb");

        var result = _engine.ExecuteOne("get users where roles contains 'admin'", "testdb");
        Assert.Equal(SproutOperation.Get, result.Operation);
    ...
    {
        _engine.ExecuteOne("upsert users {name: 'Grace', roles: ['viewer']}", "testdb");
    ...
    {
        _engine.ExecuteOne("create table scores (player string 50, values array sint)", "testdb");
        _engine.ExecuteOne("upsert scores {player: 'Alice', values: [10, 20, 30]}", "testdb");
        _engine.ExecuteOne("upsert scores {player: 'Bob', values: [5, 15]}", "testdb");

        var result = _engine.ExecuteOne("get scores where values contains '20'", "testdb");
        Assert.NotNull(result.Data);
        Assert.Single(result.Data);
    ...
    {
        _engine.ExecuteOne("upsert users {name: 'Hank', roles: ['admin']}", "testdb");
        var getResult = _engine.ExecuteOne("get users where name = 'Hank'", "testdb");
        var id = getResult.Data?[0]["_id"];
        Assert.NotNull(id);

        var deleteResult = _engine.ExecuteOne($"delete users where _id = {id}", "testdb");
        Assert.Equal(SproutOperation.Delete, deleteResult.Operation);
        Assert.Equal(1, deleteResult.Affected);

        // Verify row is gone
        var afterDelete = _engine.ExecuteOne($"get users where _id = {id}", "testdb");
    ...
    {
        var result = _engine.ExecuteOne("create index users.roles", "testdb");
    ...
    {
        var result = _engine.ExecuteOne("upsert users {name: 'Fail', roles: 'notanarray'}", "testdb");
        Assert.Equal(SproutOperation.Error, result.Operation);
    }
    ...
    public void Upsert_MultipleRecords_WithArrays()
    {
        ...
        var r1 = _engine.ExecuteOne("upsert users {name: 'Multi1', roles: ['a']}", "testdb");
        Assert.Equal(SproutOperation.Upsert, r1.Operation);

        var r2 = _engine.ExecuteOne("upsert users {name: 'Multi2', roles: ['b', 'c']}", "testdb");
        Assert.Equal(SproutOperation.Upsert, r2.Operation);

        var result = _engine.ExecuteOne("get users where name starts 'Multi'", "testdb");
        Assert.NotNull(result.Data);
    ...
    {
        _engine.ExecuteOne("upsert users {name: 'Iris', roles: ['old']}", "testdb");
        var getResult = _engine.ExecuteOne("get users where name = 'Iris'", "testdb");
        var id = getResult.Data?[0]["_id"];
        Assert.NotNull(id);

        _engine.ExecuteOne($"upsert users {{_id: {id}, roles: ['new1', 'new2']}}", "testdb");

        var updated = _engine.ExecuteOne($"get users where _id = {id}", "testdb");
