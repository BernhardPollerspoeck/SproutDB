...
public class JoinQueries : ISproutConnectionTestsSetup
{
    ...
    public void Test_GetUsers_FollowOrders_Works()
    {
        ...
        var johnOrders = resultRows.Where(r => (double)r.Fields["users.id"] == 1).ToList();
        var janeOrders = resultRows.Where(r => (double)r.Fields["users.id"] == 2).ToList();
        ...
        Assert.AreEqual(75.25d, (double)janeOrders[0].Fields[$"ordars.{TOTAL_COLUMN}"]);
    }
    ...
    public void Test_GetUsers_FollowOrdersLeft_Works()
    {
        ...
        var userIds = resultRows.Select(r => (double)r.Fields["users.id"]).Distinct().ToList();
        ...
        var aliceRow = resultRows.FirstOrDefault(r => (double)r.Fields["users.id"] == 3);
    ...
    public void Test_GetUsers_FollowOrdersOnStatus_Works()
    {
        ...
        var orderIds = resultRows.Select(r => (double)r.Fields["ordars.id"]).ToList();
    ...
    public void Test_GetUsers_FollowOrdersFollowItems_Works()
    {
        ...
        var johnRows = resultRows.Where(r => (double)r.Fields["users.id"] == 1).ToList();
        var janeRows = resultRows.Where(r => (double)r.Fields["users.id"] == 2).ToList();
    ...
    public void Test_GetUsers_FollowOrders_WhereOrderTotalGreaterThan100_Works()
    {
        ...
        var orderIds = resultRows.Select(r => (double)r.Fields["orders.id"]).ToList();
        Assert.IsTrue(orderIds.Contains(101d)); // John's order > 100
        Assert.IsTrue(orderIds.Contains(103d)); // Jane's order > 100
        Assert.IsFalse(orderIds.Contains(102d)); // John's order < 100 should not be included
    }
    ...
    public void Test_GetUsers_FollowOrders_GroupByUserName_Works()
    {
        ...
        var userNames = resultRows.Select(r => r.Fields[$"{USERS_TABLE}.{NAME_COLUMN}"]?.ToString()).ToList();
    ...
    public void Test_GetUsers_FollowOrders_GroupByUserName_HavingCountOrdersGreaterThan2_Works()
    {
        ...
        // Act
        var result = _connection.Execute($"get {USERS_TABLE} follow {USERS_TABLE}.id -> {ORDERS_TABLE}.user_id as orders group by {USERS_TABLE}.{NAME_COLUMN} having count() > 2");
        ...
        Assert.AreEqual(JOHN_NAME, resultRows[0].Fields[$"{USERS_TABLE}.{NAME_COLUMN}"]);
    }
    ...
    public void Test_GetUsers_FollowOrders_ComplexQuery_Works()
    {
        ...
        var query = $@"get {USERS_TABLE} 
                      ...
                      select {USERS_TABLE}.{NAME_COLUMN}, count() as order_count, sum(orders.{TOTAL_COLUMN}) as total_spent";
        ...
        Assert.IsTrue(row.Fields.ContainsKey($"{USERS_TABLE}.{NAME_COLUMN}"));
        Assert.IsTrue(row.Fields.ContainsKey("order_count"));
        Assert.IsTrue(row.Fields.ContainsKey("total_spent"));

        // Verify the values are correct
        Assert.AreEqual(JOHN_NAME, row.Fields[$"{USERS_TABLE}.{NAME_COLUMN}"]);
